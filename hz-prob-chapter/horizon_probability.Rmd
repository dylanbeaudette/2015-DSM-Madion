...
**Papers to cite**
* single pedon (modal) is insufficient to represent a general range in characteristics [@Jones1959]

* "any given pedon (component, series) can have a number of different realizations. re-sampling a pit, or changes in horizon designation lead to variability" [@Holmgren1988]
...
This collection of soil profile data represents the work of 13 different soil scientists, with described properties spanning ranges in physical properties (mostly related to bedrock composition). 

**incorporate this idea:** "Although a portion of the variability within morphological properties determined in the file is attributed to discrepancies in human judgement, it is assumed that a major cause of variance is due to differences in soils at different locations" [@Wilding1964]
...
# remove these columns, confusing in this context
a.ml$confidence <- NULL 
p.ml$confidence <- NULL

# combine ML horizonation tables
hz_bnd_tbl <- cbind(a.ml, p.ml)

# adjust names and print ML horizonation tables
