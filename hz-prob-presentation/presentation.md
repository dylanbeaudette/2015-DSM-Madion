...
$P[Y \geq j | X]$ is the probability of encountering horizon $j$, conditioned by depth, $X$, based on an intercept for every possible horizon, $\alpha_{j}$, and single coefficient, $\beta$. RCS basis functions accomodate non-linear relationships between $X$ and $P$.

## Shannon Entropy (H index)
$$ H = -\sum_{i=1}^{n}{p_{i} * ln(p_{i})}  $$
$H$ is an index of uncertainy associated with the probabilities, $\mathbf{p}$, of encountering horizons $i$ through $n$ at some depth. Larger values suggest **more** confusion (less information).
